...
class HttpTools{
	...
	static public function curl_put($resource,$formParams,$username,$access_token,$clientIp){
		...
		$authParams =  array(
			...
			"client_id"=> C("RESTFULL.client_id"),
			"timestamp"=>self::microtime_float()
		);
		$header[] = "Rest-Auth:".self::gs(C("RESTFULL.restapi_URL").$resource, $authParams, $formParams);
		$header[] = "FD:".urlencode(self::getValuePair($formParams));
		$list = self::curl(C("RESTFULL.restapi_URL").$resource, $header, $formParams, "PUT");
		
		if(self::$isDebug)
		{
			echo "==========put请求开始============<br><br>";
			echo "put请求路径：".C("RESTFULL.restapi_URL").$resource."<br>";
	...
	static public function curl_post($resource,$formParams,$username,$access_token,$clientIp){
		...
		$header[] = "Rest-Auth:".self::gs(C("RESTFULL.restapi_URL").$resource, $authParams, $formParams);
		$header[] = "FD:".urlencode(self::getValuePair($formParams));
		$list = self::curl(C("RESTFULL.restapi_URL").$resource, $header, $formParams, "POST");
		
		if(self::$isDebug)
		{
			echo "==========post请求开始============<br><br>";
			echo "post请求路径：".C("RESTFULL.restapi_URL").$resource."<br>";
	...
	static public function curl_get($resource,$bizParams,$username,$access_token,$clientIp){
	    ...
	    {
	        $header[] = "Rest-Auth:".self::gs(C("RESTFULL.restapi_URL").$resource."?".self::getValuePairToGs($bizParams),$authParams, array());
	    }
	    else
	    {
	        $header[] = "Rest-Auth:".self::gs(C("RESTFULL.restapi_URL").$resource, $authParams, array());
	    }
	    $header[] = "FD:"."";

	    if(count($bizParams)>0)
	    {
	        $list = self::curl(C("RESTFULL.restapi_URL").$resource."?".self::getValuePairToGs($bizParams), $header, array(), "GET");
	    }
	    else
	    {
	        $list = self::curl(C("RESTFULL.restapi_URL").$resource, $header, array(), "GET");
	    }

	    if(self::$isDebug)
	    {
	    	echo "==========get请求开始============<br><br>";
	    	echo "get请求路径：".C("RESTFULL.restapi_URL").$resource."<br>";
	    	echo "鉴权头信息：".print_r($header,true)."<br>";
			echo "请求结果：".print_r($list,true)."<br><br>";
	    	echo "==========get请求结束============<br><br>";
	    }
	...
	static public function curl_delete($resource,$bizParams,$username,$access_token,$clientIp){
	    ...
	    if(count($bizParams)>0)
	    {
	        $list = self::curl(C("RESTFULL.restapi_URL").$resource."?".self::getValuePairToGs($bizParams), $header, array(), "DELETE");
	    }
	    else
	    {
	        $list = self::curl(C("RESTFULL.restapi_URL").$resource, $header, array(), "DELETE");
	    }
	    
	    if(self::$isDebug)
	    {
	    	echo "==========delete请求开始============<br><br>";
	    	echo "delete请求路径：".C("RESTFULL.restapi_URL").$resource."<br>";
	    	echo "鉴权头信息：".print_r($header,true)."<br>";
			echo "请求结果：".print_r($list,true)."<br><br>";
	...
	static function gs($fullPath, $authParams,$formParams)
	{
		...
		$amd5 = md5($clientid.$tmsp.C("RESTFULL.appkey").$access_token.$username);
